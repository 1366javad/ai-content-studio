...
export async function PUT(req: Request) {
  ...
  const maxTokens = Math.min(4000, Math.max(100, Math.round(Number(body.maxTokens ?? 2000) / 100) * 100));
  const suggestionsEnabled = Boolean(body.suggestionsEnabled);

  SETTINGS = { theme, preferredModel, temperature, maxTokens, suggestionsEnabled } as any;
