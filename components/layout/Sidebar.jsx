...
const sidebarItems = [
  ...
  },
  {
    ...
    icon: <RiBarChartLine className="w-6 h-6 text-blue-400" />,
  },
  {
    name: "Settings",
        ...
        <nav className="flex-1 p-3">
          {sidebarItems.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className={`flex items-center  px-4 py-3 md:py-4 rounded-xl transition-all duration-200 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 m-2 ${pathname === item.href ? "bg-gray-200 dark:bg-gray-800" : ""}`}
            >
